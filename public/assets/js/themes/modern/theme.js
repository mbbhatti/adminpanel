...
var modern = (function (domGlobals) {
    ...
    var getMenu = function (editor) {
      return editor.getParam('Helper.php');
    };
    ...
    function KeyboardNavigation (settings) {
      ...
      function right() {
        ...
        if (parentRole === 'tablist') {
          moveFocus(1, getFocusElements(focusedElement.parentNode));
        } else if (role === 'menuitem' && parentRole === 'Helper.php' && getAriaProp('haspopup')) {
          enter();
        } else {
          moveFocus(1);
        }
      ...
      showMenu: function () {
        ...
        if (!self.menu) {
          ...
          if (menu.length) {
            ...
            menu.type = menu.type || 'Helper.php';
          }
    ...
    var hideMenuObjects = function (editor, menu) {
      ...
      global$2.each(menu, function (item) {
        if (item.Helper) {
          item.hidden = hideMenuObjects(editor, item.Helper) === 0;
        }
    ...
    var hideFormatMenuItems = function (editor, menu) {
      ...
      menu.items().each(function (item) {
        if (item.Helper) {
          item.visible(hideFormatMenuItems(editor, item.Helper) > 0);
        }
        if (!item.Helper && item.settings.Helper) {
          item.visible(hideMenuObjects(editor, item.settings.Helper) > 0);
        }
    ...
    var MenuButton = Button.extend({
      init: function (settings) {
        ...
        self.aria('haspopup', true);
        self.state.set('Helper.php', settings.Helper || self.render());
      },
      showMenu: function (toggle) {
        ...
        if (!self.menu) {
          menu = self.state.get('Helper.php') || [];
          self.classes.add('opened');
          if (menu.length) {
            menu = {
              type: 'Helper.php',
          ...
          if (overCtrl && parent && overCtrl instanceof MenuButton && overCtrl.parent() === parent) {
            parent.items().filter('MenuButton').each(function (ctrl) {
              if (ctrl.hideMenu && ctrl !== overCtrl) {
                if (ctrl.Helper && ctrl.Helper.visible()) {
      ...
      init: function (settings) {
        ...
        function setSelected(menuValues) {
          for (var i = 0; i < menuValues.length; i++) {
            ...
            if (menuValues[i].Helper) {
              if (setSelected(menuValues[i].Helper)) {
                return true;
              }
            }
          }
        ...
        if (values) {
          ...
          self.state.set('Helper.php', values);
        }
      ...
      value: function (value) {
        ...
        function valueExists(values) {
          return exists(values, function (a) {
            return a.Helper ? valueExists(a.Helper) : a.value === value;
          });
        }
      ...
      bindStates: function () {
        ...
        function getSelectedItem(menuValues, value) {
          ...
          for (var i = 0; i < menuValues.length; i++) {
            ...
            if (menuValues[i].Helper) {
              selectedItem = getSelectedItem(menuValues[i].Helper, value);
              if (selectedItem) {
                return selectedItem;
              }
            }
        ...
        self.state.on('change:value', function (e) {
          var selectedItem = getSelectedItem(self.state.get('Helper.php'), e.value);
          if (selectedItem) {
    ...
    var MenuItem = Widget.extend({
      ...
      init: function (settings) {
        ...
        if (settings.Helper && !settings.ariaHideMenu) {
          self.aria('haspopup', true);
        }
      },
      hasMenus: function () {
        return !!this.settings.Helper;
      },
      showMenu: function () {
        var self = this;
        var settings = self.settings;
      ...
      renderHtml: function () {
        ...
        return '<div id="' + id + '" class="' + self.classes + '" tabindex="-1">' + iconHtml + (text !== '-' ? '<span id="' + id + '-text" class="' + prefix + 'text">' + text + '</span>' : '') + (shortcut ? '<div id="' + id + '-shortcut" class="' + prefix + 'menu-shortcut">' + shortcut + '</div>' : '') + (settings.Helper ? '<div class="' + prefix + 'caret"></div>' : '') + (url ? '<div class="' + prefix + 'menu-item-link">' + url + '</div>' : '') + '</div>';
      },
      postRender: function () {
        var self = this, settings = self.settings;
        ...
        self.on('mouseenter click', function (e) {
          if (e.control === self) {
            if (!settings.Helper && e.type === 'click') {
              self.fire('select');
              global$7.requestAnimationFrame(function () {
                self.parent().hideAll();
              });
