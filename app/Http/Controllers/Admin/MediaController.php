...
class MediaController extends Controller
{
    ...
    {
        $this->filesystem = config('storage.disk');
    }
    ...
    public function createFolder(Request $request)
    {
        ...
        if (Storage::disk($this->filesystem)->exists($new_folder)) {
            $error = __('Sorry that folder already exists, please delete that folder if you wish to re-create it');
        } elseif (Storage::disk($this->filesystem)->makeDirectory($new_folder)) {
            $success = true;
        } else {
            $error = __('Sorry something seems to have gone wrong with creating the directory, please check your permissions');
        }
        ...
        foreach ($request->get('files') as $file) {
            ...
            if ($file['type'] == 'folder') {
                if (!Storage::disk($this->filesystem)->deleteDirectory($file_path)) {
                    $error = __('Sorry something seems to have gone wrong when deleting this folder, please check your permissions');
                    $success = false;
                }
            } elseif (!Storage::disk($this->filesystem)->delete($file_path)) {
                $error = __('Sorry something seems to have gone wrong deleting this file, please check your permissions');
                $success = false;
            }
        }
    ...
    public function rename(Request $request)
    {
        ...
        if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
            if (Storage::disk($this->filesystem)->move("{$location}/{$filename}", "{$location}/{$newFilename}")) {
                ...
                $error = __('Sorry there seems to be a problem moving that file/folder, please make sure you have the correct permissions.');
            }
        } else {
            $error = __('File or Folder may already exist with that name. Please choose another name or delete the other file.');
        }
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $allowedMimeTypes = config('food.media.allowed_mimetypes', '*');
            if ($allowedMimeTypes != '*' && (is_array($allowedMimeTypes) && !in_array($request->file->getMimeType(), $allowedMimeTypes))) {
                throw new Exception(__('This mimetype is not allowed'));
            }
            ...
            $success = true;
            $message = __('Successfully uploaded new file!');
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $success = true;
            $message = __('Successfully cropped the image');
        } catch (Exception $e) {
            $success = false;
            $message = $e->getMessage();
        }
