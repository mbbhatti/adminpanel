                ...
                <div class="file_link selected" aria-hidden="true" data-toggle="tooltip" data-placement="auto" :title="file">
                    <div class="link_icon">
                        <template v-if="fileIs(file, 'image')">
                            <div class="img_icon" :style="imgIcon('{{ Storage::disk(config('storage.disk'))->url('/') }}'+file)"></div>
                        </template>
    ...
    <div id="toolbar" v-if="showToolbar" :style="expanded ? 'display:block' : 'display:none'">
        ...
        <div class="btn-group offset-right">
            <button type="button" :disabled="selected_files.length == 0" v-if="allowUpload && hidden_element" class="btn btn-default" v-on:click="addSelectedFiles()">
                <i class="voyager-upload"></i>Add all selected
            </button>
        ...
        <div class="flex">
            <div id="left">
                ...
                <div id="file_loader" v-if="is_loading">
                    <?php $admin_loader_img = setting('admin.loader', ''); ?>
                    @if($admin_loader_img == '')
                        <img src="{{ asset('assets/images/logo-icon.png') }}" alt="Voyager Loader">
                    @else
                        <img src="{{ Storage::disk(config('storage.disk'))->url($admin_loader_img) }}" alt="Voyager Loader">
                    @endif
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <h5 class="folder_warning">
                        ...
                        Deleting a folder will remove all files and folders contained inside
                    </h5>
                </div>
    ...
    Vue.component('media-manager', {
        ...
        methods: {
            getFiles: function() {
                var vm = this;
                vm.is_loading = true;
                $.post('{{ route('media.files') }}', { folder: vm.current_folder, _token: '{{ csrf_token() }}', details: vm.details }, function(data) {
                    vm.files = [];
                ...
                if (file.type != 'folder') {
                    if (!this.allowMultiSelect) {
                        ...
                        if (content.length >= this.maxSelectedFiles && this.maxSelectedFiles > 0) {
                            var msg_sing = "{{ trans_choice('You can only select one file|You can only select a maximum of :count files', 1) }}";
                            var msg_plur = "{{ trans_choice('You can only select one file|You can only select a maximum of :count files', 2) }}";
            ...
            createFolder: function(e) {
                ...
                $.post('{{ route('media.new_folder') }}', { new_folder: vm.current_folder+'/'+name, _token: '{{ csrf_token() }}' }, function(data) {
					if(data.success == true){
            ...
            /*crop: function(mode) {
				...
				$.post('{{ route('media.crop') }}', postData, function(data) {
					if (data.success) {
                    ...
                    if (vm.hidden_element) {
                        if (vm.maxSelectedFiles > 1) {
                            ...
                            if (content.length < vm.minSelectedFiles) {
                                e.preventDefault();
                                var msg_sing = "{{ trans_choice('You must select at least one file|You must select at least :count files', 1) }}";
                                var msg_plur = "{{ trans_choice('You must select at least one file|You must select at least :count files', 2) }}";
                            ...
                            if (vm.minSelectedFiles > 0 && vm.hidden_element.value == '') {
                                e.preventDefault();
                                toastr.error("{{ trans_choice('You must select at least one file|You must select at least :count files', 1) }}");
                            }
                        }
                    }
                });
